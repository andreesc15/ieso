#!/usr/bin/env python3
# ieso_mc_numba.py  –  NumPy-/Numba-accelerated Monte-Carlo engine for Indonesian ESOs
# (Algorithm identical to Chendra et al. 2022; only performance plumbing changed)
# ---------------------------------------------------------------------
...
# try importing numba; fall back gracefully if unavailable
try:
...
# ──────────────────────────────────────────────────────────────────────
# 2.  Core model class (public API unchanged)
# ──────────────────────────────────────────────────────────────────────
class IESOMonteCarlo:
    """
    Numba-accelerated Indonesian ESO Monte-Carlo engine.
    Usage identical to the original version; all improvements are internal.
    """
    # ----- constructor ---------------------------------------------------
    def __init__(
        self,
    ...
    def price(
        ...
        Only difference: heavy inner loops are executed by Numba.
        """
...
# ──────────────────────────────────────────────────────────────────────
# 3. Quick numerical smoke test (unchanged)
# ──────────────────────────────────────────────────────────────────────
if __name__ == "__main__":
    ...
    p1, ci1 = eso1.price(n_paths=1000000, verbose=True)
    ...
    p2, ci2 = eso2.price(n_paths=1000000, verbose=True)
    ...
    eso8 = IESOMonteCarlo(maturity_years=5.0, vesting_years=1.0,
                          S0=5000, r=0.05, q=0.025, sigma=0.30,
                          lam=0.06, M=1.0, seed=7, debug=False)
    p8, ci8 = eso8.price(n_paths=1000000, verbose=True)
